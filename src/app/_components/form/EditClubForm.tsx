...
import styles from './EditClubForm.module.scss';
import { Typography } from '../typography';
import Image from 'next/image';
import LogoSmall from '@/app/_assets/LogoSmall';
import ClubCardSkeleton from '../skeleton/ClubCardSkeleton';
...
export default function EditClubForm({ clubData }: { clubData: any }) {
  ...
  const [department, setDepartment] = useState(
    departmentToKorean[clubData.department],
  );
  const [lightLogo, setLightLogo] = useState(clubData.logo);
  const [newLightLogo, setNewLightLogo] = useState<string | null>(null);
  const [newDarkLogo, setNewDarkLogo] = useState<string | null>(null);
  const [darkLogo, setDarkLogo] = useState(clubData.logo_dark);
  const [clubName, setClubName] = useState(clubData.displayName);
  const [clubSubName, setClubSubName] = useState(clubData.name);
  const [clubLocation, setClubLocation] = useState(clubData.room);
  const [clubDescription, setClubDescription] = useState(clubData.description);
  const [darkMode, setDarkMode] = useState(false);
  const [facebook, setFacebook] = useState(clubData.facebook);
  const [instagram, setInstagram] = useState(clubData.instagram);
  const [website, setWebsite] = useState(clubData.homepage);
  ...
  const handleDarkLogo = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onloadend = () => {
      ...
      if (previewImgUrl) {
        setNewDarkLogo(previewImgUrl as string);
      }
    };
  };
  ...
  const handleLightLogo = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onloadend = () => {
      ...
      if (previewImgUrl) {
        setNewLightLogo(previewImgUrl as string);
      }
    };
  };
  ...
  return (
    <div className={styles.layout}>
      ...
      <form action={editClub} className={styles.form}>
        <input type="hidden" name="logo" value={lightLogo} />
        <input type="hidden" name="logo_dark" value={darkLogo} />
                ...
                {lightLogo ? (
                  <label htmlFor="logo" className={styles['input-image']}>
                    <Image
                      width="80"
                      height="80"
                      src={newLightLogo ? newLightLogo : lightLogo}
                      alt="light-logo"
                      className={styles['image-preview']}
                    />
                  </label>
                ...
                {darkLogo ? (
                  <label htmlFor="logo_dark" className={styles['input-image']}>
                    <Image
                      width="80"
                      height="80"
                      src={newDarkLogo ? newDarkLogo : darkLogo}
                      alt="dark-logo"
                      className={styles['image-preview']}
                    />
                  </label>
